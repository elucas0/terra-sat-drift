...
def to_segmentation_prediction(y):
    y_hat = y.output
...
class DomainAdaptationTask(SemanticSegmentationTask):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        y_target = self.squeeze_ground_truth(target_batch["mask"])
        
        # 2. Extraction des caractéristiques via le backbone
        # On récupère le token [CLS] (index 0) de la dernière couche
        feat_source = self.model.encoder(x_source)[-1][:, 0, :]
        feat_target = self.model.encoder(x_target)[-1][:, 0, :]

        # 3. Calcul des pertes
        model_output = self(x_target)
        seg_loss_dict = self.train_loss_handler.compute_loss(
            model_output, y_target, self.criterion, self.aux_loss
        )
        self.train_loss_handler.log_loss(self.log, loss_dict=seg_loss_dict, batch_size=y_target.shape[0])
        seg_loss = seg_loss_dict["loss"]
        
        y_hat_hard = to_segmentation_prediction(model_output)
        self.train_metrics.update(y_hat_hard, y_target)
        ...
        self.log("train/mmd_dist", mmd_dist, prog_bar=True)
        self.log("train/total_loss", total_loss, prog_bar=True)
        return total_loss

    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        """Validate on target domain data only."""
        # During validation, we receive batches from dm_target.val_dataloader()
        # which is a standard single batch, not a combined loader
        x = batch["image"]
        y = self.squeeze_ground_truth(batch["mask"])
        
        # Utilisation de l'inférence native (gère le tiling éventuel)
        other_keys = batch.keys() - {"image", "mask", "filename"}
        rest = {k: batch[k] for k in other_keys}
        model_output = self.handle_full_or_tiled_inference(x, self.tiled_inference_on_validation, **rest)
        
        # Calcul et log via le val_loss_handler original
        ...
        # Mise à jour des métriques de validation (indispensable pour l'EarlyStopping basé sur mIoU)
        y_hat_hard = to_segmentation_prediction(model_output)
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        """Test on target domain data only."""
        # During testing, we receive batches from dm_target.test_dataloader()
        # which is a standard single batch, not a combined loader
        x = batch["image"]
        y = self.squeeze_ground_truth(batch["mask"])
        ...
        if dataloader_idx >= len(self.test_loss_handler):
            raise ValueError("You are returning more than one test dataloader but not defining enough test_dataloaders_names.")
            
        loss_dict = self.test_loss_handler[dataloader_idx].compute_loss(model_output, y, self.criterion, self.aux_loss)
